...
이 문서는 장기 설계 문서이고, 현재 코드베이스는 일부가 이미 반영된 상태다.

현재 반영된 항목:

- `ScheduleType` 기반 일정 타입 관리
- `calendar_patterns`와 `calendar_weekly_rules`를 함께 사용하는 구조
- `교대 근무 캘린더`와 `일반 일정 캘린더` 분리
- 일반 일정 캘린더용 주간 반복 규칙
- 캘린더 다중 선택 UX
- `내 캘린더` / `공유받은 캘린더` / 소유자별 그룹화
- 공유 그룹용 스키마와 1차 UI

아직 설계 단계로 남아 있는 항목:

- `calendar_patterns`와 향후 규칙 모델 통합
- 공유 모델의 완전한 단일화
- 테스트와 마이그레이션 정리
...
- 패턴 빌더가 여전히 교대 근무 중심으로 설계되어 있다.
- 일반 일정 캘린더는 지원하지만, 교대 캘린더만큼 전용 편집 경험이 풍부하지 않다.
- 코드 라벨, 시간 범위, 범례는 상당 부분 일반화됐지만 여전히 교대 근무 흔적이 남아 있다.
...
- 현재 코드는 `calendar_patterns`와 `calendar_weekly_rules`를 분리해서 사용하고 있다.
- 장기적으로는 이 둘을 `calendar_rules` 같은 공통 모델로 통합하는 것이 목표다.
...
- 아직 실제 운영 코드에서 사용 중이다.
- 일반 일정 캘린더는 `calendar_weekly_rules`, 교대 캘린더는 `calendar_patterns` 중심으로 동작한다.
