...
- Phase 1 완료
  - `내 캘린더` / `공유받은 캘린더` 그룹화
  - API 응답에 ownership 그룹 정보 추가
- Phase 2A 완료
  - 다중 캘린더 진입 시 캘린더 선택 화면 추가
- Phase 2 완료
  - 공유받은 캘린더 소유자별 그룹화
- Phase 3 1차 완료
  - `share_groups`, `share_group_members`, `calendar_share_grants` 마이그레이션 추가
  - 엔티티/리포지토리/서비스/컨트롤러 추가
  - 접근 권한 계산에 `직접 공유 + 그룹 공유` 반영
- Phase 4 1차 완료
  - 공유 관리 화면에 그룹 생성/멤버 관리/그룹 권한 부여 UI 추가
  - 공유 관리 화면의 현재 캘린더 기준 UX 정리

현재 미완료:

- 사용자 초대와 그룹 권한을 하나의 접근 권한 목록으로 통합 표시
- 공유 출처 직접 표시
- 기존 `calendar_shares`와 `calendar_share_grants` 정리 전략 확정
- 테스트 보강
...
- 공유 그룹용 DB/엔티티/리포지토리/서비스/컨트롤러 추가
- 직접 공유 + 그룹 공유 권한 계산 반영
- 공유 관리 UI 1차 반영

## Follow-up Checklist

- 공유 관리 화면에서 사용자 초대 목록과 그룹 권한 목록 통합
- 공유 출처 직접 표시
- 직접 공유와 그룹 공유의 권한 병합 테스트 추가
- `calendar_shares`와 `calendar_share_grants` 통합 전략 정리
